...
void				calcTorYValue(double* xPoints, double* yPoints, EdgePoint& torPoint, double& avgTorY, double& avgFiveEightY, double& avgTorY_Xval, double& webX, double& webY);
...
double LEFT_LASER_ROTATION_ANGLE = 0.0;
double RIGHT_LASER_ROTATION_ANGLE = 0.0;
...
//GLOBALS
double g_ip_CV2_IMAGE_MAX_X_1;
double g_ip_CV2_IMAGE_MAX_Y_1;
double g_ip_CV2_IMAGE_MIN_X_1;
double g_ip_CV2_IMAGE_MIN_Y_1;
double g_ip_CV2_IMAGE_MAX_X_2;
double g_ip_CV2_IMAGE_MAX_Y_2;
double g_ip_CV2_IMAGE_MIN_X_2;
double g_ip_CV2_IMAGE_MIN_Y_2;
...
ImageProcessing::ImageProcessing(double ip_CV2_IMAGE_MAX_X_1, double ip_CV2_IMAGE_MAX_Y_1, double ip_CV2_IMAGE_MIN_X_1, double ip_CV2_IMAGE_MIN_Y_1, double ip_CV2_IMAGE_MAX_X_2, double ip_CV2_IMAGE_MAX_Y_2, double ip_CV2_IMAGE_MIN_X_2, double ip_CV2_IMAGE_MIN_Y_2)
{
	g_ip_CV2_IMAGE_MIN_X_1 = ip_CV2_IMAGE_MIN_X_1;
	g_ip_CV2_IMAGE_MIN_Y_1 = ip_CV2_IMAGE_MIN_Y_1;
	g_ip_CV2_IMAGE_MAX_X_1 = ip_CV2_IMAGE_MAX_X_1;
	g_ip_CV2_IMAGE_MAX_Y_1 = ip_CV2_IMAGE_MAX_Y_1;
	g_ip_CV2_IMAGE_MIN_X_2 = ip_CV2_IMAGE_MIN_X_2;
	g_ip_CV2_IMAGE_MIN_Y_2 = ip_CV2_IMAGE_MIN_Y_2;
	g_ip_CV2_IMAGE_MAX_X_2 = ip_CV2_IMAGE_MAX_X_2;
	g_ip_CV2_IMAGE_MAX_Y_2 = ip_CV2_IMAGE_MAX_Y_2;

	double image_size_x_1 = CV2_IMAGE_SIZE_X_1;
	double image_size_y_1 = CV2_IMAGE_SIZE_Y_1;
	double image_max_x_1 = ip_CV2_IMAGE_MAX_X_1;
	double image_max_y_1 = ip_CV2_IMAGE_MAX_Y_1;
	double image_min_x_1 = ip_CV2_IMAGE_MIN_X_1;
	double image_min_y_1 = ip_CV2_IMAGE_MIN_Y_1;
	...
	double image_max_x_2 = ip_CV2_IMAGE_MAX_X_2;
	double image_max_y_2 = ip_CV2_IMAGE_MAX_Y_2;
	double image_min_x_2 = ip_CV2_IMAGE_MIN_X_2;
	double image_min_y_2 = ip_CV2_IMAGE_MIN_Y_2;
...
void rotateXYPoints(double* xVals, double* yVals, double calibrationAngle, int capturedPoints, double* xRotated, double* yRotated)
{
	...
	{
		double calTheta = calibrationAngle * 3.14159265 / 180.0;//NEED RADIANS
		double resTheta = calTheta - 3.14159264 / 2.0;
	...
	TemplatePoints& lastBestPoint, EdgePoint& tmpTOR, bool& angleMatch, bool& gradMatch)
{
		...
		if (templates[i].leftX > 0)
		{
			...
			for (int j = 0; j < 2048; j++)
			{
					...
					&& yPoints[j] * -1.0 <= approxCorner_y + TOR_CIRCLE_MATCH_RADIUS)
				{
					...
					if (secondSlopes[j] <= -60.0 && secondSlopes[j] >= -100.0)
					{
					...
					if (firstAngles[j] >= 20.0 && firstAngles[j] <= 80.0)
					{
	...
	localTOR.houghCenterX = templates[0].xValue;
	localTOR.houghCenterY = templates[0].yValue;
	localTOR.xValue = templates[0].xProfilePoint;
	localTOR.yValue = templates[0].yProfilePoint;
	angleMatch = templates[0].angleMatch;
	gradMatch = templates[0].gradMatch;
	tmpTOR = localTOR;
	return torCnt;
}
...
void calcTorYValue(double* xPoints, double* yPoints, EdgePoint& torPoint, double& avgTorY, double& avgFiveEightY, double& avgTorY_Xval, double& webX, double& webY)
{
	...
	double sum = 0.0;
	double cnt = 0.0;
	double torXpointSum = 0.0;

	//FIND THE RAIL'S WEB X VALUE
	webX = 0.0;
	webY = 0.0;
	for (int i = 0; i < 2048; i++)
	{
		if (xPoints[i] >= torPoint.xValue + 0.5 * 25.4 && xPoints[i] <= torPoint.xValue + 2.0 * 25.4
			&& yPoints[i] >= torPoint.yValue - 4.0 * 25.4 && yPoints[i] <= torPoint.yValue - 1.0 * 25.4)
		{
			if (webY < xPoints[i])
			{
				webX = xPoints[i];
	...
	//CALCULATE TOR WITH THE 'MEAN' METHOD
	double meanCnt = 0.0;
	double meanSum = 0.0;
	double meanTorXpointSum = 0.0;
	for (int i = 0; i < 2048; i++)
	{
		...
		{
			meanCnt = meanCnt + 1.0;
			meanSum = meanSum + yPoints[i];
			meanTorXpointSum = meanTorXpointSum + xPoints[i];
		}
	}

	//IF WE 'DID-NOT' FOUND AN X-VALUE FOR THE WEB
	if (webX == 0.0)
	{
		cnt = meanCnt;
		sum = meanSum;
		torXpointSum = meanTorXpointSum;
	}
	else//IF WE FOUND AN X-VALUE FOR THE WEB
	{
		//EXPAND SEARCH AREA -- COUNT = 0
		while (foundPoints < 5 && tries < 10)
		{
			foundPoints = 0;
			cnt = 0.0;
			sum = 0.0;
			for (int i = 0; i < 2048; i++)
			{
				if (xPoints[i] >= webX + 5.25 + 3.0 + 3.0 - tries * 1.5 && xPoints[i] <= webX + 8.4 + 3.0 + 3.0 + tries * 1.5
					&& yPoints[i] >= torPoint.yValue - 15/* WAS -30  3/6/2020*/ && yPoints[i] <= torPoint.yValue + 20)
				{
					foundPoints++;
					cnt = cnt + 1.0;
					sum = sum + yPoints[i];
					torXpointSum = torXpointSum + xPoints[i];
				}
			}
			tries = tries + 1.0;
		}
	...
	if (foundPoints >= 1 && cnt >= 1.0 && sum >= 1.0)
	{
		avgTorY = sum / cnt;
		avgTorY_Xval = torXpointSum / cnt;
	}
	else
	{
		//USE THE MEAN VALUES
		avgTorY = meanSum / meanCnt;
		avgTorY_Xval = meanTorXpointSum / meanCnt;
	}

	double avgVal = avgTorY - 15.88 - 1.5 - 0.5;
...
void calcFiveEightXValue(double* xPoints, double* yPoints, EdgePoint& torPoint, double avgFiveEightY, double& avgFiveEightX)
{
	...
	for (int i = 0; i < 2048; i++)
	{
		if (xPoints[i] <= torPoint.xValue + TOR_CIRCLE_MATCH_RADIUS
			...
			&& yPoints[i] >= avgFiveEightY - 1.58 && yPoints[i] <= avgFiveEightY + 1.0)//+/- 1/16"=1.58
		{
			cnt = cnt + 1.0;
			sum = sum + xPoints[i];
		}
...
unsigned __stdcall ImageProcessing::analyzeProfile(void* args)
{
	...
	{
	case 1:
		imageMinX = g_ip_CV2_IMAGE_MIN_X_1;
		imageMaxX = g_ip_CV2_IMAGE_MAX_X_1;
		imageMinY = g_ip_CV2_IMAGE_MIN_Y_1;
		imageMaxY = g_ip_CV2_IMAGE_MAX_Y_1;
		scale = cv2Sensor1Scale;
		flip = FLIP_IMAGE_1;
		rotationAngle = LEFT_LASER_ROTATION_ANGLE;//CALIBRATION_ANGLE_1;
		break;

	case 2:
		imageMinX = g_ip_CV2_IMAGE_MIN_X_2;
		imageMaxX = g_ip_CV2_IMAGE_MAX_X_2;
		imageMinY = g_ip_CV2_IMAGE_MIN_Y_2;
		imageMaxY = g_ip_CV2_IMAGE_MAX_Y_2;
		scale = cv2Sensor2Scale;
		flip = FLIP_IMAGE_2;
		rotationAngle = RIGHT_LASER_ROTATION_ANGLE;//CALIBRATION_ANGLE_2;
		break;
	}
	...
	{
		////DETERMINE MEAN Y-VALUE; +/-39mm FROM TOR POINT
		calcTorYValue(rotatedPoints.xVals, rotatedPoints.yVals, tmpTOR, avgTopRail_Y, fiveEight_Y, avgTopRail_X, webX, webY);
	...
	points.torAvgY_Xval = avgTopRail_X;
...
//SET THE LASER CALIBRATION ANGLES FOR ROTATION CORRECTION
void ImageProcessing::setLaserCorrectionAngles(double leftLaser, double rightLaser)
{
	RIGHT_LASER_ROTATION_ANGLE = rightLaser;
	LEFT_LASER_ROTATION_ANGLE = leftLaser;
}

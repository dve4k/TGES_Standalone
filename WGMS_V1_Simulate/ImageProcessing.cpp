...
void calcTorYValue(double* xPoints, double* yPoints, EdgePoint& torPoint, double& avgTorY, double& avgFiveEightY, double& avgTorY_Xval, double& webX, double& webY)
{
	...
	////FIND THE RIGHT SIDE OF RAIL TOP
	//double outsideTop_x = 0;
	//double outsideTop_y = 0;
	//for (int i = 0; i < 2048; i++)
	//{
	//	if (xPoints[i] >= torPoint.xValue + 5 && xPoints[i] <= torPoint.xValue + 75.0
	//		&& yPoints[i] >= torPoint.yValue - 30 && yPoints[i] <= torPoint.yValue + 20)
	//	{
	//		if (outsideTop_x < xPoints[i])
	//		{
	//			outsideTop_x = xPoints[i];
	//			outsideTop_y = yPoints[i];
	//		}
	//	}
	//}
	...
	//ESIMATE MIDDLE OF RAIL
	//double estMidX = outsideTop_x - 37.0;
	...
	//int middleIndex = (int)floor((int)torYVals.size() * .5);
	...
	//	{
	//		if (yPoints[l] == median && medianXVal == 0)
	//		{
	//			medianXVal = xPoints[l];
	//		}
	//		avgTorY_Xval = medianXVal;
	//	}
	//	avgTorY = median; // OVERWRITE AVERAGE WITH THE MEDIAN
	//}
	//double avgVal = avgTopRail_Y - 15.8;
	double avgVal = avgTorY - 15.88 - 1.5 - 2.0;
